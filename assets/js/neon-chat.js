...
;(function($, window, undefined)
{
	...
	$.extend(neonChat, {
		...
		open: function($user_link)
		{
			...
			var id = $user_link.attr('id');

			$conversation_header.find('.display-name').html('<a href="./profile?id='+id+'" class="whiteLink">'+$user_link.find('em').text()+'</a>');
		...
		{
			var msg = $.trim($textarea.val()).trim();
			...
			if(this.isOpen && this.$current_user)
			{
				...
				if (msg.length > 0) {
					Server.send('message', JSON.stringify({'id':id,'message':msg}));
