...
<div class="page-container chat-visible">
	<div class="main-content">
		...
		<div class="row">
			<?php
			$LoginRegister->UserExist($_GET['id']);
			echo $LoginRegister->ShowImages($_GET['id']);
			if (isset($_POST['AddFriend'])) {
				echo $LoginRegister->AddFriend($_GET['id']);
			}
			else if (isset($_POST['RemoveFriend'])) {
				echo $LoginRegister->RemoveFriend($_GET['id']);
			}
			else if (isset($_POST['CancelRequest'])) {
				echo $LoginRegister->CancelRequest($_GET['id']);
			}
			else if (isset($_POST['AcceptRequest'])) {
				echo $LoginRegister->AcceptRequest($_GET['id']);
			}
			echo $LoginRegister->CheckFriends($_GET['id']);
			?>
		</div>
		<div class="row">
		<?php
			//echo $LoginRegister->profilestatus($_GET['id']);
		?>
		...
		<?php
			echo $LoginRegister->ShowUsersPosts($_GET['id']);
		?>
		</div>
	</div>
...
<script type="text/javascript" charset="utf-8">
	Raven.config('https://b1832eb7dcf842c089c39da5a0dbace0@sentry.io/178049').install();
